...
public interface TitleBarAction {
     ...
     * 设置标题栏默认的标题
     */
    default void setDefaultTitle(@StringRes int id) {
        setDefaultTitle(getTitleBar().getResources().getString(id));
    }

    /**
     * 设置标题栏默认的标题
     */
    default void setDefaultTitle(CharSequence title) {
        if (isToolbar()) getTitleBar().setTitle(title);
    }
    ...
    default int getMenuSize(){
        if(isToolbar()) return getTitleBar().getMenu().size();
        return 0;
    }
    ...
    default void setMenuTitle(int index,String title){
        if(isToolbar()) getTitleBar().getMenu().getItem(index).setTitle(title);
