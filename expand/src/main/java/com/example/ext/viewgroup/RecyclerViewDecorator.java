...
/**
 * 装饰RecyclerView，原生不支持添加头部和尾部，装饰增强可添加头部、尾部功能
 */
public class RecyclerViewDecorator extends RecyclerView {
    private Adapter mAdapter;//原来的Adapter
    private final AdapterDecorator mAdapterDecorator = new AdapterDecorator();
    public RecyclerViewDecorator(@NonNull Context context) {
        super(context);
    }

    public RecyclerViewDecorator(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }

    public RecyclerViewDecorator(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    public void setAdapter(@Nullable Adapter adapter) {
        mAdapter = adapter;
        mAdapterDecorator.setReplaceAdapter(adapter);
        super.setAdapter(mAdapterDecorator);
    }
    ...
    public void addHeaderView(View view) {
        mAdapterDecorator.addHeaderView(view);
    }
    /**添加尾部*/
    @SuppressWarnings("unchecked")
    ...
    public void addFooterView(View view) {
        mAdapterDecorator.addFooterView(view);
    }
    /**
        ...
        if (layoutManager instanceof GridLayoutManager) {
            ((GridLayoutManager) layoutManager).setSpanSizeLookup(new GridLayoutManager.SpanSizeLookup() {
                ...
                public int getSpanSize(int position) {
                    return (position < mAdapterDecorator.getHeaderViewCount()
                            || position >= mAdapterDecorator.getHeaderViewCount() + (mAdapter == null ? 0 : mAdapter.getItemCount()))
     ...
     * 装饰Adapter
     */
    private static final class AdapterDecorator extends Adapter<ViewHolder>{
        ...
        private AdapterDataObserverDecorator mObserver;
        ...
        private void setReplaceAdapter(Adapter adapter) {
            ...
            if(mAdapter != adapter) {
                ...
                mAdapter = adapter;
                if(mAdapter != null) {
                    if(mObserver == null) mObserver = new AdapterDataObserverDecorator(this);
     ...
     * 参照源码
     */
    private static final class AdapterDataObserverDecorator extends AdapterDataObserver {

        private final AdapterDecorator mAdapterDecorator;

        private AdapterDataObserverDecorator(AdapterDecorator adapter) {
            mAdapterDecorator = adapter;
        }

        @Override
        public void onChanged() {
            mAdapterDecorator.notifyDataSetChanged();
        }
        ...
        public void onItemRangeChanged(int positionStart, int itemCount, Object payload) {
            mAdapterDecorator.notifyItemRangeChanged(mAdapterDecorator.getHeaderViewCount() + positionStart, itemCount,payload);
        }
        ...
        public void onItemRangeInserted(int positionStart, int itemCount) {
            mAdapterDecorator.notifyItemRangeInserted(mAdapterDecorator.getHeaderViewCount() + positionStart, itemCount);
        }
        ...
        public void onItemRangeRemoved(int positionStart, int itemCount) {
            mAdapterDecorator.notifyItemRangeRemoved(mAdapterDecorator.getHeaderViewCount() + positionStart, itemCount);
        }
        ...
        public void onItemRangeMoved(int fromPosition, int toPosition, int itemCount) {
            mAdapterDecorator.notifyItemMoved(mAdapterDecorator.getHeaderViewCount() + fromPosition, toPosition);
        }
    }
}
